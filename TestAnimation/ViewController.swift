...
//  TestAnimation
//   git 并不会实时的跟踪你的文件 只有你明确让他记录你的文件时，他才会把指定的文件的当前状态记录到仓库中去，所有一般当我们修改完文件的时候需要先git add fileName1 fileName2 如果你嫌麻烦的话可以直接git add -A 添加所有的文件到暂存的区域 当添加完之后 你可以用 git commit 到编辑窗口里面输入备注完成提交  以上的步骤如果嫌麻烦的话可以直接git commit -am 备注 完成提交到本地仓库
...
// 新加了一个备注   合并的时候可能是更具某个时间节点的提交进行合并判断的 如 master分支在10.2修改了a文件 test分支在10.3修改了a文件 当将master分支合并到b分支的时候 a文件进行合并的时候应该就会采取最近的时间点的引用
